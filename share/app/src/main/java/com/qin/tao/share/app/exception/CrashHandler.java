package com.qin.tao.share.app.exception;
...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ...
        //				Log.e(TAG, "Error : ", e);
        //			}
        //			Intent intent = MainApplication.context_App.getPackageManager().getLaunchIntentForPackage(MainApplication.context_App.getPackageName());
        //			intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        //			MainApplication.context_App.startActivity(intent);
        ...
        //重启应用
        Intent intent = BaseApplication.appContext.getPackageManager().getLaunchIntentForPackage(BaseApplication.appContext.getPackageName());
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    private boolean handleException(Throwable ex) {
        ...
        //			{
        //				Looper.prepare();
        //				//				ToastUtil.makeText(mContext, "程序出错啦:" + msg, Toast.LENGTH_LONG);
        //				Looper.loop();
        //			}
        //
        //		}.start();
    ...
    private String saveCrashInfoToFile(Throwable ex) {
        ...
        try {
            if (SDCardUtils.isAvailable()) {
                File file = new File(SDCardConfig.APPLICATION_SYSTEM_LOG);
                SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH-mm-ss");
                if (!file.exists())
                    file.mkdirs();
                String timestamp = format.format(new Date(System.currentTimeMillis()));
                fileName = file.getAbsolutePath() + "/crash-" + timestamp + CRASH_REPORTER_EXTENSION;
