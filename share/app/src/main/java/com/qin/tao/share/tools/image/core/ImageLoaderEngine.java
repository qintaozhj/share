...
class ImageLoaderEngine {
     ...
     * 检查相同 uri 是否在同一 imageAware 上重复加载显示
     */
    protected boolean checkImageAwareUriOnTag(ImageAware imageAware, String uri) {
        boolean returnValue = false;
        if (imageAware != null && imageAware.getWrappedView() != null && URLUtil.isNetworkUrl(uri)) {
            Object tagUri = imageAware.getWrappedView().getTag(R.id.image_loader_uri);
            if (tagUri instanceof String) {
                returnValue = uri.trim().equals(tagUri.toString().trim());
                //				if (!returnValue)
                //				{
                //					Logger.i("checkImageAwareUriOnTag", uri);
                //					Logger.i("checkImageAwareUriOnTag1", tagUri.toString());
                //				}
            }
        }
        return returnValue;
    ...
    protected void setImageAwareUriToTag(ImageAware imageAware, String uri) {
        if (imageAware != null && imageAware.getWrappedView() != null) {
            imageAware.getWrappedView().setTag(R.id.image_loader_uri, uri);
        }
    }
}
