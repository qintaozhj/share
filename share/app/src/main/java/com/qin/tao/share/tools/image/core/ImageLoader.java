 ...
 *******************************************************************************/
package com.qin.tao.share.tools.image.core;
...
import com.qin.tao.share.tools.image.cache.disc.DiskCache;
import com.qin.tao.share.tools.image.cache.memory.MemoryCache;
import com.qin.tao.share.tools.image.core.assist.FailReason;
import com.qin.tao.share.tools.image.core.assist.FlushedInputStream;
import com.qin.tao.share.tools.image.core.assist.ImageSize;
import com.qin.tao.share.tools.image.core.assist.LoadedFrom;
import com.qin.tao.share.tools.image.core.assist.ViewScaleType;
...
import com.qin.tao.share.tools.image.core.imageaware.NonViewAware;
import com.qin.tao.share.tools.image.core.listener.ImageLoadingListener;
import com.qin.tao.share.tools.image.core.listener.ImageLoadingProgressListener;
import com.qin.tao.share.tools.image.core.listener.SimpleImageLoadingListener;
import com.qin.tao.share.tools.image.utils.ImageSizeUtils;
import com.qin.tao.share.tools.image.utils.L;
...
public class ImageLoader
{
							 ...
							 ImageLoadingProgressListener progressListener)
	{
		...
		if (configuration.disabledToDuplicateLoadUriToDisplayImageAware && !options.isDuplicateLoadUriToDisplay())
		{
			if (engine.checkImageAwareUriOnTag(imageAware, uri))
				return;
		}
		...
		if (bmp != null && !bmp.isRecycled())
		{
			...
			} else
			{
				...
				if (this.configuration.disabledToDuplicateLoadUriToDisplayImageAware)
					engine.setImageAwareUriToTag(imageAware, uri);
    ...
    public void displayImage(int drawableResId, ImageView imageView) {
        this.displayImage(drawableResId, imageView, null);
    }
     ...
     * @param options
     */
    public void displayImage(int drawableResId, ImageView imageView, DisplayImageOptions options) {
        if (drawableResId > 0)
            this.displayImage("drawable://" + drawableResId, imageView, options);
    }
     ...
     */
    public void displayLocalResource(String photoPath, ImageView imageView, DisplayImageOptions options) {
        if (!TextUtils.isEmpty(photoPath))
            this.displayImage("file://"+photoPath,imageView,options);
    }

    /**
     * 根据制定尺寸加载本地图片并显示,并在指定tagId上对比文件路径是否相同,相同的才加载
     * @param photoPath
     * @param resources
     * @param targetView
     * @param width
     ...
     * @param targetId
     */
    public void loadAndDisplayLocalResourceWithSize(final String photoPath, final Resources resources, final ImageView targetView, int width, int height, final DisplayImageOptions options, final int targetId) {
        if (!TextUtils.isEmpty(photoPath)){
            this.loadImage("file://" + photoPath, width > 0 && height > 0 ? new ImageSize(width, height) : null, options, new ImageLoadingListener() {
                @Override
                public void onLoadingStarted(String imageUri, View view) {
                    if (imageUri.equalsIgnoreCase("file://"+targetView.getTag(targetId)))
                        targetView.setImageDrawable(options.getImageOnLoading(resources));
                }

                @Override
                public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
                    if (imageUri.equalsIgnoreCase("file://"+targetView.getTag(targetId)))
                        targetView.setImageDrawable(options.getImageOnFail(resources));
                }

                @Override
                public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
                    if (imageUri.equalsIgnoreCase("file://"+targetView.getTag(targetId)))
                        targetView.setImageBitmap(loadedImage);
                }
